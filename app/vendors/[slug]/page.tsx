...
import { getAppsByVendorId, getVendorBySlug } from "@/lib/mock/repository";
...
export default function VendorPage({ params }: VendorPageProps) {
  ...
  const apps = getAppsByVendorId(vendor.id);

  return (
    <section className="space-y-6">
      <header>
        <h1 className="text-3xl font-semibold text-slate-900">{vendor.name}</h1>
        <p className="mt-2 text-slate-600">{vendor.description}</p>
      </header>
